...
if userName.lower() == "professor quigley":
    print(f"\nMy creator, {userName}. Pleasure to servce you!")
else:
    print(f"\nHello, {userName}. Nice to meet you!")

keepGoing = "y"
while keepGoing.lower() == "y":
    size = input("\nWhat size do you want? Enter small, meduim, or large:  ")
    while size.lower() not in ["small", "meduim", "large"]:
        size = input("Invalid value!  Please enter small, medium, or large:  ")

    flavor = input("\nEnter the flavor of pizza:  ")
    while len(flavor) == 0:
        flavor = input("Flavor cannot be blank!  Please enter crust type:  ")
